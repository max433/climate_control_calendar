...
class MultiCalendarCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        for calendar_id in self.calendar_entities:
            try:
                ...
                if calendar_state.state == "on":
                    # Calendar is active - extract current event
                    ...
                    # Calendar is off - no active event
        ...
        # Log summary
        _LOGGER.debug(
            "Calendar update complete | Total events: %d | Active events: %d | Errors: %d",
            len(all_events),
            len(active_events),
            len(errors),
        )
    ...
    def _extract_event_from_state(
        ...
        """
        Extract event data from calendar state.
